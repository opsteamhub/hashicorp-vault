...
resource "aws_autoscaling_group" "failure_analysis_ecs_asg_vault" {
  ...
  target_group_arns         = [aws_lb_target_group.tg_vault.arn, aws_lb_target_group.tg_vault_cluster.arn, aws_lb_target_group.tg_exporter.arn]
...
resource "aws_autoscaling_group" "failure_analysis_ecs_asg_vault_replica" {
  ...
  target_group_arns         = [aws_lb_target_group.tg_vault_replica[0].arn,aws_lb_target_group.tg_vault_cluster_replica[0].arn]
  #health_check_type         = "ELB"
  desired_capacity          = var.desired_capacity
  min_size                  = var.min_size
  max_size                  = var.max_size
